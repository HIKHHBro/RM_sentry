																															...
																															,uint8_t on_off)
{
	...
	addr->rx_buff_data = (uint8_t *)malloc(addr->rx_buff_size * sizeof(uint8_t)); //分配接收缓存地址空间
	/* -------- 使能 --------- */
...
//      DMA2->LIFCR=(1<<21)|(1<<20);//清楚完成标志位
		  HAL_UART_Receive_DMA(huartx,addr->rx_buff_data,addr->rx_buff_size);//设置接收地址和数据长度
      return HAL_OK;
		}
/*---------------------------------80字符限制-----------------------------------*/
	/**
