...
**Data**  : *2019-04-09 17:32*   **author**: *HIKpingtouge*    **log**:      
  1. 修改云台模式切换或者启动时，出现翘头现象，因为垂直的真实在是在8000多
  2. 修改云台位置环pid计算出现溢出
  3. 修改云台pitch误差计算的过零阀值
**Data**  : *2019-04-10 03:35*   **author**: *HIKpingtouge*    **log**:      
  1. 增加小电脑单目测距
  2. 用MATLAB拟合摩擦轮pwm值和射频的映射，距离和射频的映射  待测试和重新取点
  3. 增加遥控和小电脑数据接收帧率检测  待测试和重新取点
**Data**  : *2019-04-10 10:26*   **author**: *HIKpingtouge*    **log**:      
  1. 改用获取两次接收的时间做帧率办法成用软定时固定时间更新帧率办法，更新时间要不能太长，太长延迟严重
